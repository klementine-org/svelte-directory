<script module lang="ts">
	...
	class PaginationParams {
		...
		constructor(count: number) {
			this.#limitStore = queryParam(PaginationParamEnum.LIMIT, ssp.number(PAGINATION_DEFAULTS.limit));
...
<script>
	...
	// Styling
	const letterClasses = 'flex flex-col items-stretch justify-center text-3xl font-medium select-none';
	...
	const paginationClasses = 'text-muted-foreground cursor-pointer data-selected:cursor-default data-selected:text-primary hover:text-primary disabled:cursor-not-allowed disabled:text-muted-foreground';
	const ellipsisClasses = 'text-muted-foreground cursor-default text-[15px] font-medium';
</script>

<Pagination.Root count={paginationParams.count} perPage={paginationParams.perPage} bind:page={paginationParams.page}>
		...
		<div class="my-8 flex  items-center">
			...
			<div class="flex items-baseline gap-0.5" aria-hidden="true">
				...
				{#each pages as page (page.key)}
					...
					{:else}
						<Pagination.Page
							class={cn(letterClasses, paginationClasses, "data-selected:[&>*:nth-child(2)]:text-foreground data-selected:[&>*:nth-child(2)]:no-underline!")}
							{page}>
							<span>e</span>
							<span class="text-sm font-normal text-blue-500 select-text hover:underline">{page.value}</span>
						</Pagination.Page>
					{/if}
				{/each}
